...
\end{equation}
against the specifications $\mathrm{MA} \in [-1.25, +2]$\,nm and $\mathrm{MSD} \leq 7$\,nm of Chapter~\ref{ch:control_objectives}. These are, by construction, the same numbers as the Case-1 row of Table~\ref{tab:exp4_ladder} in Experiment 4: both now come from the identical full-model code path, so Experiment 1 and the controller-ladder baseline are no longer two different plants reported side by side. Figure~\ref{fig:exp1_ma_msd} shows both indices over the sequence and Figure~\ref{fig:exp1_zoom} the detail of the first exposure window.
...
The single snap bound still walks the entire wafer across the specification, though the corrected plant needs it roughly an order of magnitude tighter than in the earlier, mass-bugged draft of this experiment. At the baseline snap the exposure MSD is 994\,nm and every die fails; the classifier reads the saturated map as \textit{Center}, an out-of-domain morphology it prices at maximum. Tightening the snap by successive factors of two to five ($s = 2\times10^4$, $10^4$, $5\times10^3$) lowers the MSD to 355, 174, and 63.1\,nm, and the whole wafer is still failing at every one of these points --- unlike the earlier estimate, $s = 5\times10^3$ no longer clears the bound at all. Only at $s = 10^3$ does the MSD reach single digits (9.5\,nm), still 2.5\,nm over the 7\,nm bound and therefore still a 49/49 failure; the specification is finally cleared at $s = 100$ (1.0\,nm, 0 of 49 failing, \textit{none}), at the price of a scan lengthened from 161 to 785\,ms. The MA band $[-1.25, +2]$\,nm remains far out of reach: at the finest tested snap, $s = 50$, $\mathrm{MSD} = 0.5$\,nm comfortably clears the MSD spec but $\mathrm{MA} = 28.7$\,nm is more than an order of magnitude above the band, and the wafer now takes 933\,ms per scan. The crossing point for the full specification (MA and MSD both) therefore lies well below the finest grid point tested here, and considerably further below it than the earlier estimate suggested.

This is the central mechanism of the thesis, made quantitative: on this two-stage machine the exposure-window MSD specification is reachable through the \textit{trajectory} alone, by bounding the snap, and throughput is the price --- a steeper price on the corrected plant than first estimated, since clearing the same 7\,nm bound now costs roughly a tenfold tighter snap and a wafer cycle time of 48\,s rather than 31\,s. The moving-average band is considerably further away still, and its crossing point is not resolved by the present grid. It is not the servo that closes the gap. Raising the actuator-mount stiffness toward the reference machine's rigid value, or the fine loop's bandwidth beyond its $K_P/K_V = 100$\,rad/s pole, both \textit{degrade} the tracking rather than improve it: the residual is the structural ring that the bandwidth-limited loop cannot suppress but that a sufficiently smooth reference never excites. That is why the optimization of Experiment 6 searches the kinematic bounds under the CNN cost --- the specification defines an achievable operating point, and the trajectory that reaches it most cheaply, or that trades a scanner-plausible yield loss for throughput, is what the optimizer must find.
...
\textbf{Under the full ladder, trajectory order is still worth roughly a factor of thirty.} With the feedforward removing the quasi-static lag and the HIGS loop suppressing what lies within its bandwidth, the cruise MSD falls $24.0 \rightarrow 3.27 \rightarrow 0.834$\,\textmu m from second to fourth order --- a $\approx29\times$ improvement attributable to the profile alone, at identical kinematic bounds, and $\approx77\times$ to the tight-snap variant (312\,nm). The interpretation is classical and now quantified for this (corrected) plant: the fine loop handles the error components inside its bandwidth, and the profile order determines how much excitation exists \textit{beyond} it; smoothing the reference and improving the controller are complements, not substitutes. Two further readings matter for the optimizer. First, smoothness has a throughput price --- $T_{scan}$ grows 21\,\% from second to fourth order at the same bounds, and a further 30\,\% for the tight-snap variant, both unchanged from the plant correction since profile timing is purely kinematic --- so profile order participates in the same throughput--yield trade as the kinematic bounds themselves. Second, the tight-snap variant remains the \textit{most} accurate configuration in the cruise (MSD 312\,nm, MA 2.32\,\textmu m --- smaller than every other configuration of this plant, though now well short of the nanometre specifications rather than close to them, a reflection of the corrected plant's overall coarser floor), yet its full-scan MSD is no better than the plain fourth-order profile's (4.96 vs.\ 4.59\,\textmu m, both set by the post-step entry transient) while it spends 30\,\% more scan time. Accuracy inside the cruise keeps improving with smoothness, while the settling-dominated component saturates at the closed loop's floor and the throughput cost grows without bound: the optimum in the snap dimension is therefore interior once throughput enters the cost --- precisely the trade the optimizer of Experiment 6 is built to resolve.
...
For the Case 1 configuration as simulated, the honest answer is more qualified than it would be for a genuinely reduced model, because this work uses none: every result, including Case 1's, comes from the full 16-body MuJoCo model. What still holds is the argument of Chapter~\ref{ch:model} (Section~\ref{sec:model_dynamics}, Eq.~\eqref{eq:accented} et seq.): at the frozen operating condition $\dot\theta_z = \ddot\theta_z = 0$ that a pure step-and-scan sequence never leaves, the corrections that make the full machine a quasi-LPV system vanish identically, its equations coincide with the reference study's LTI form, and Case 1 --- collocated PID, no saturation exercised at these bounds, no fine-stage nonlinearity in the loop --- never excites the terms that would break that equivalence. So the map from reference kinematics to synchronization error \textit{is} an exact LTI operator here, in principle capturable by a fitted low-order transfer function with no training data and full interpretability; it simply must now be evaluated by integrating the same full multibody model as everywhere else (tens of seconds per candidate, Experiment 1), rather than a separately coded reduced form. A GRU trained on one hundred randomized runs approximates something that, in this regime, can in principle be computed exactly from first principles; its only measured advantage is evaluation speed against the full simulation, and that advantage is smaller than it would be against a genuinely reduced model, since there is no such cheaper alternative in this work to begin with. The training data also transferred a defect into the surrogate: the synchronization-error signal it learned from contains the constant geometric offset of the absolute coordinates, which is why the predictions of Table~\ref{tab:exp4_lstm} rank correctly but carry a meaningless absolute scale.
